...
class DetectionModel(torch.nn.Module):
    def __init__(self, num_actions, input_size=258):
        ...
        # We don't use softmax because we use CrossEntropyLoss which applies softmax
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x, _ = self.lstm3(x)
        x = self.relu(x[:, -1, :])  # Shape is now (batch_size, input_size) |We only take the last element of the LTSM sequence
